...
function getCategoryList() {
    ...
    $.ajax({
        ...
        dataType: "json",
        type: 'GET',
        url: "/api/speak/Category.php?action=getCategoryList",
...
function f_initGrid() {
        ...
        columns: [
        { display: '分类编号', name: 'id', minWidth: 50, type: 'text' },
...
function getEdit() {
    ...
    if (!selected) {
       ...
       $.ligerDialog.open({
            name: 'updatePage',
            url: 'CategoryEdit.html?operation=update&categoryId='+selected.id,
            height: 200,
            width: 500,
            title: '修改课本信息',
            buttons: [{
        ...
        $.ligerDialog.confirm('确定删除?', function (flag) {
            if (flag) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    type: 'POST',
                    url: "/api/speak/Category.php?action=deleteCategory",
                    loading: '正在进行删除...',
