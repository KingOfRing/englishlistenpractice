...
function f_initTree(){
    ...
    $("#tree").ligerTree({
        url:"/api/speak/Article.php?action=getBookMenu",
        idFieldName: 'id',
        slide: false,
...
function f_initGrid() {
        ...
        columns: [
        { display: '文章编号', name: 'id', minWidth: 50, type: 'text' },
        { display: '课本编号', name: 'bookId', minWidth: 200, type: 'text' },
        { display: '文章标题', name: 'articleTitle', minWidth: 200, type: 'text' },
        { display: '作  者', name: 'authorName', minWidth: 200, type: 'text' },
        { display: '介  绍', name: 'description', minWidth: 200, type: 'text' },
...
function getArticleByBookId() {
    var response;
    $.ajax({
        async: true,
        dataType: "json",
        type: 'GET',
        url: "/api/speak/Article.php?action=getArticleList&bookId="+bookId,
        success: function (data,status) {
           if(data['resCode'] == 'S456'){
                rowData.Rows = data.articleList;
...
function getInsert() {
    $.ligerDialog.open({
        name: 'insertPage',
        url: 'ArticleEdit.html?operation=insert&bookId='+bookId,
        height: 300,
        width: 600,
        title: '新增文章',
        buttons: [{
...
function getEdit() {
    ...
    if (!selected) {
       ...
       $.ligerDialog.open({
            name: 'updatePage',
            url: 'ArticleEdit.html?operation=update&bookId='+bookId+'&articleId='+selected.id,
            height: 300,
            width: 600,
            title: '修改文章信息',
            buttons: [{
        ...
        $.ligerDialog.confirm('确定删除?', function (flag) {
            if (flag) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    type: 'POST',
                    url: "/api/speak/Article.php?action=deleteArticle",
                    loading: '正在进行删除...',
