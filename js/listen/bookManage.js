...
function f_initTree(){
    ...
    $("#tree").ligerTree({
        url:"/api/listen/Book.php?action=getCategoryMenu",
        idFieldName: 'id',
        slide: false,
...
function f_initGrid() {
        ...
        columns: [
        { display: '课本编号', name: 'bookId', minWidth: 50, type: 'int' },
        ...
        // { display: '封  面', name: 'imageUrl', minWidth: 200, type: 'text' },
...
function getBookByCategoryId() {
    ...
    $.ajax({
        ...
        dataType: "json",
        type: 'GET',
        url: "/api/listen/Book.php?action=getBookList&categoryId="+categoryId,
...
function getEdit() {
    ...
    if (!selected) {
       ...
       $.ligerDialog.open({
            name: 'updatePage',
            url: 'BookEdit.html?operation=update&bookId='+selected.bookId,
            height: 450,
            width: 700,
            title: '修改课本信息',
            buttons: [{
...
function getDelete()  {
    ...
    if (!selected) {
        ...
        for(i = 0;i<checkedRows.length;i++)
            selectedRows += checkedRows[i].bookId + ",";
        ...
        $.ligerDialog.confirm('确定删除?', function (flag) {
            if (flag) {
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    type: 'POST',
                    url: "/api/listen/Book.php?action=deleteBook",
                    loading: '正在进行删除...',
